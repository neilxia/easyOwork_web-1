function sendreportCtrl() {
	return [
			...
			function($scope, $modal, $compile, $state, roleService, MsgService,
					reportService, LocalStorage, Common) {
				...
				function inquiryCreatedReports() {
					...
					var promise = reportService.inquiryCreatedReports({
						body : $scope.options
					});
										...
										promise
												.success(function(data, status,
													...
													if (sts.statusCode == 0) {
														...
														MsgService
																.tomsg(data.body.status.errorDesc);
													}
												});
										promise
	...
	return [
			...
			function($scope, $modal, $compile, $state, roleService, MsgService,
				...
				function inquiryAssignedReports() {
					...
					var promise = reportService.inquiryAssignedReports({
						body : $scope.options
					});
					promise.success(function(data, status, headers, config) {
						var sts = data.body.status;
	...
	return [
			...
			function($scope, $modal, $compile, $state, roleService, MsgService,
				...
				$scope.initFun = function() {
					...
					if (selectedReport != undefined && selectedReport != null) {
						...
						if ($scope.report.month != null)
							$scope.report.month = new Date($scope.report.year,
									$scope.report.month - 1, 1);
						if ($scope.report.year != null)
							$scope.report.year = new Date($scope.report.year,
									0, 1);
					}
				};
				...
				$scope.getTask = function() {
					...
					function modalCtrl($scope, $modalInstance, report) {
						...
						var promise = taskService.inquiryAssignedTasks({
							body : $scope.options
						});
						promise
						...
						var promise1 = projectService.inquiryProjectTasks({
							body : $scope.options1
						});
						promise1
								.success(function(data, status, headers, config) {
									var sts = data.body.status;
									if (sts.statusCode == 0) {
										$scope.projectTasklist = data.body.data.projectTasks;
									} else {
									}
								});
						...
						$scope.clicktask = function(row) {
							$scope.selectedTask = row;
						};
						
						$scope.add = function() {
							if($scope.selectedTask != null)
								report.content = report.content + '\r\n\r\n' + $scope.selectedTask.title + "\r\n" + $scope.selectedTask.content;
						};
						$scope.cancel = function() {
							$modalInstance.dismiss('cancel');
						};
						$scope.clickprojecttask = function(row) {
							$scope.selectedProjectTask = row;
						};
						
						$scope.addprojecttask = function() {
							if($scope.selectedProjectTask != null)
								report.content = report.content + '\r\n\r\n' + $scope.selectedProjectTask.taskName + "\r\n" + $scope.selectedProjectTask.description;
						};
					}
				}
				...
				$scope.submit = function() {
					...
					if ($scope.theapply && $scope.theapply.selectedallarr) {
						...
						if ($scope.theapply.selectedallarr[1].length > 0) {
							angular
									.forEach(
											$scope.theapply.selectedallarr[1],
											function(val, ind) {
												userDTOList
														.push({ // 适用人员
						...
						if ($scope.theapply.selectedallarr[0].length > 0) {
							angular
									.forEach(
											$scope.theapply.selectedallarr[0],
											function(val, ind) {
												orgDTOList
														.push({ // 适用角色
					...
					var promise = reportService.changeReport({
						body : $scope.options
					});
					promise.success(function(data, status, headers, config) {
						var sts = data.body.status;

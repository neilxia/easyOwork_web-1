(function($){
	...
	var methods = {
		...
		createContainer:function(){
			...
			$inner.append("<table id='"+id+"' class='table table-hover' cellspacing=0 cellpadding=0 />");
		...
		drawHeader:function(){
			...
			if(config.showCheckbox ){
				//第一列要用来显示checkbox
				$tr.append("<td width='50' class='t_c bre'><input type='checkbox' trid='"+headerId+"' /></td>");
			}
			...
			if(config.showbtnbox ){
				//最后列要用来显示btn
				$tr.append("<td width="+config.showbtnwidth+">操作</td>");
			}
			return this.each(function(){
		...
		drawDataTd:function($tr){
			...
			if(config.showbtnbox ){
				//最后列要用来显示btn
				var btns=config.showbtnboxcom;
				var btnhtml='';
				if(btns.length<=0){
					return;
				}else{
					$.each( btns, function(i, btn){
						btnhtml = btnhtml+'<button class="'+btn.classbox+'" title="'+btn.title+'">'+btn.text+'</button>';
					});
				}
		...
		bindEvent:function(){
			...
			//bind click to button.btn
			$context.on('click','button.btn', function(){
				var $this=$(this);
				var $thistr = $this.parents('tr');
				var id = $thistr.attr('id');
				var data = $thistr.data("data");
				$.each(config.showbtnboxcom,function(i,obj){
					if($this[0].title==obj.title && obj.btnClick){
						obj.btnClick(id,data);
					}
				});
		...
		bindCheckboxEvent:function(){
			...
			$("input[type='checkbox'][trid]").on('ifChecked',function (event){
				//阻止事件冒泡
			...
			$("input[type='checkbox'][trid]").on('ifUnchecked',function (event){
				//阻止事件冒泡
				event.stopPropagation();
				var $ck = $(this);
				var checked = this.checked;
...
/*			$context.on('ifChanged',"input[type='checkbox'][trid]",function(event){
		...
		toggleCheckRecursive:function(id,status){
			...
			//改变当前行的状态
			//debugger;
			$context.find('#'+id).find("input[type='checkbox'][trid]").iCheck(status?'check':'uncheck'); //nose
			//$context.find('#'+id).find("input[type='checkbox'][trid]").prop('checked',status);
			//递归处理子行
		...
		uncheckParentRecursive:function(id,status){
			...
			if(!status && pid!=undefined ){
				...
				$ptr.find("input[type='checkbox'][trid]").iCheck(status?'check':'uncheck');
